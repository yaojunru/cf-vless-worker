...
3. Authenticate with `npx wrangler login`, then confirm the intended account with `npx wrangler whoami`.
4. Generate a UUID locally and choose a unique path at least eight characters long. Never write either secret to tracked files, shell history, CI logs, or client screenshots.
5. Set secrets interactively:
...
6. Deploy with `npm run deploy`. Bind a custom domain only after the Worker URL works. Keep the DNS record proxied.
7. Confirm that `/` is a `404` and that the exact configured path only upgrades with a WebSocket request. Use a VLESS client to test an actual TCP connection.
8. Record the Worker version ID shown by Wrangler. Roll back with `npx wrangler rollback <version-id>` when the deployment regresses.
